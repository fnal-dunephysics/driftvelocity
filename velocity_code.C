#define velocity_code_cxx
#include "velocity_code.h"
#include "driftvel.h"
...
void velocity_code::Loop()
{
  ...
  /* TFile *dist_map=new TFile("Zdist_3D_histograms_test.root");
  TH3F *Zcorrection=(TH3F*)dist_map->Get("Zdist_3D");
  TH3F *Ycorrection=(TH3F*)dist_map->Get("Ydist_3D");*/
  ...
  TFile *crossing_dist=new TFile("Zdist_3D_histograms_test.root");
  ...
  TFile *f=new TFile("velocity_data_assymerrors_jan4_2022_test_vel_allevents.root","RECREATE");
  ...
  //for(Long64_t jentry=0;jentry<=4000000;jentry++){
    ...
    if(t_neg_buff.size() && t_pos_buff.size()){
      //  std::cout<<"inside 1st loop "<<std::endl;
	...
	//	std::cout<<"inside 2nd loop "<<std::endl;
   	...
   	double dT=t_max_pos-t_min_pos+t_max_neg-t_min_neg;
	//	std::cout<<" dT value is "<<dT<<std::endl;
	delta_tmin->Fill(t_min_pos-t_min_neg);
	//	std::cout<<"In the 2nd loop "<<std::endl;
   	if(!(abs(t_min_pos-t_min_neg)<10 && dT>9000 && dT<9220 && t_max_pos<5985 && t_max_neg<5985)) continue;
   	//if(!(dT>9120 && dT<9160)) continue;
	...
	if(xvalue[k].size()>5){
	  ...
	  // std::cout<<"size for velocity vector "<<tvalue[k].size()<<std::endl;
	  TGraph *gr=new TGraph(tvalue[k].size(),&tvalue[k][0],&xvalue[k][0]);
	  // std::cout<<"test18a "<<std::endl;
	  gr->Draw();
	  // gr->Fit("pol1","Q");
	  // std::cout<<"test18b "<<std::endl;
	  gr->Fit("pol1","Q");
	  TF1 *f1=gr->GetFunction("pol1");
	  if(count_1<50){
	    canv[count_1]=new TCanvas(Form("c%d",count_1),Form("c%d",count_1));
	    canv[count_1]->cd();
	    gr->Draw();
	    t1[count_1].SetNDC();

	    gr->SetTitle(";t_{drift} [ticks=0.5 #mus];X ccordinate [cm];");
	    ...
	    double velocity_value=abs(20.0*f1->GetParameter(1));
	    t1[count_1].DrawLatex(0.30,0.84,Form("drift velocity = %0.3f mm/#mus",velocity_value));
	    canv[count_1]->Draw();
	    // canv[count_1]->Write();
	    canv[count_1]->Print(Form("gr_%d.pdf",count_1));
	    gr->Draw("AP");
	    //gr->Write(Form("gr_vel_%d",count_1));
	    count_1++;
	  }
	  // std::cout<<"test18c "<<std::endl;
	 
	  // std::cout<<"test18d "<<std::endl;
	  double slope=f1->GetParameter(1);
	  // std::cout<<"test18e "<<std::endl;
	  velocities[k].push_back(abs(20.0*slope));
	  //std::cout<<"test18f "<<std::endl;
